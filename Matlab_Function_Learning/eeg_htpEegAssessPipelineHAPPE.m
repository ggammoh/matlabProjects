...
% input parsing
defaultOutputDir = '/srv/Analysis/Sri_Projects/Matlab_Function_Learning/';%setting output directory
defaultResampleRate = 500;% resampling rate set at 500
defaultGroupLabels = {EEG1.setname,EEG2.setname};%sets the gruop labels as EEG1 and EEG2
defaultTableOnly = false;
defaultSaveOutput = true;
defaultOutputFile = [];

ip = inputParser();
addRequired(ip, 'EEG1', @isstruct);%checks to see if EEG1 is a structure
addRequired(ip, 'EEG2', @isstruct);%checks to see if EEG2 is a structure
addParameter(ip, 'outputdir', defaultOutputDir, @isfolder);%adds the following parameters to the ip structure
...
parse(ip, EEG1, EEG2, varargin{:});%will parse the 3 structures 
